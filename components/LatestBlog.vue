<template>
        <div class="featured-card__container font-primary py-20 md:py-40" :style="{ backgroundImage: `url('${featuredImage }')` }">
            ...
            <div class="inner w-auto container relative z-10 mx-5 md:mx-auto max-w-4xl bg-white py-10 px-5 md:px-10 md:py-20">
                ...
                <div class="text-md md:text-xl" v-html="LatestPost.hero[0].description"></div>
                <div class="flex flex-col text-center md:text-left md:flex-row mt-5 md:mt-10">
                    <nuxt-link class="py-4 px-10 mb-4 md:mb-0 md:mr-4 border border-black transition-all duration-500 ease-in-out hover:bg-black hover:text-white" :to="'blog/' + LatestPost.slug">Read Now</nuxt-link>
                    <nuxt-link class="py-4 px-10 border border-black transition-all duration-500 ease-in-out hover:bg-black hover:text-white" to="blog">View All</nuxt-link>
                </div>
            </div>
        </div>
</template>
